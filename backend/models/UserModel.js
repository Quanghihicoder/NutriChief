...
export const updateUserInfo = (data, result) => {
    ...
    if (data.user_gender == 0) { // female 
        tdee = 10 * data.user_weight + 6.25 * data.user_height - 5 * (currentYear - data.user_year_of_birth) - 161
    } else if (data.user_gender == 1) { // male 
        tdee = 10 * data.user_weight + 6.25 * data.user_height - 5 * (currentYear - data.user_year_of_birth) + 5
    }
    ...
    db.query("UPDATE user SET user_name = ?, user_year_of_birth = ?, user_gender = ?, user_weight = ?, user_height = ?, user_activity_level = ?, user_bmi = ?, user_tdee = ?  WHERE user_id = ?", [data.user_name, data.user_year_of_birth, data.user_gender, data.user_weight, data.user_height, data.user_activity_level, bmi, tdee, data.user_id], (err, results) => {
        if (err) {
            console.log(err);
            result({ "status": 0, "message": "Can not update user", "data": [] });
        } else {
            result({ "status": 1, "message": "Successfully update user", "data": [] });
        }
    });
}